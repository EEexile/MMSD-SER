#!/usr/bin/env python3
"""
MMER软标签训练脚本 - 自蒸馏 (Self-Distillation) 5-Fold 交叉验证版
...
from data_mmer_soft import SoftLabelMMERDataset, collate_fn_soft_mmer
...
def train_5fold_soft_distillation(output_dir, args):
    """5-Fold 交叉验证软标签训练 - 对齐版"""
    set_seed(args.seed)
    device = torch.device(args.device if torch.cuda.is_available() else 'cpu')
    
    logging.info("="*60)
    logging.info("Starting 5-Fold Self-Distillation Training (Aligned Output with BT-CL)")
    logging.info("="*60)
    ...
    logging.info(f"\n{'='*60}")
    logging.info("5-Fold Cross-Validation Results (Soft Distillation)")
    logging.info(f"{'='*60}")
    ...
    for res in fold_results:
        logging.info(f"Fold {res['fold']}: WA={res['wa']:.2f}%, UA={res['ua']:.2f}%, W-F1={res['wf1']:.2f}% (Ep:{res['epoch']})")
...
def main():
    parser = argparse.ArgumentParser(description='MMER 5-Fold Self-Distillation Training')
    ...
    now = datetime.now()
    output_dir = os.path.join("outputs", "soft_distillation_5fold", now.strftime("%Y-%m-%d"), now.strftime("%H-%M-%S"))
    ...
    train_5fold_soft_distillation(output_dir, args)
