...
def train_single_fold(target_fold, output_dir, seed=42):
    """单独运行指定的某个Fold"""
    set_seed(seed)
    ...
    logging.info(f"Training mode: Single Fold ({target_fold})")
    ...
    logging.info(f"\n{'='*60}")
    logging.info(f"Running Fold {target_fold} - Using Session {target_fold} as test/val set")
    logging.info(f"{'='*60}")
    
    # 划分数据集
    test_indices = session_indices[target_fold]
    train_indices = []
    for s in range(1, 6):
        if s != target_fold:
            train_indices.extend(session_indices[s])
            
    logging.info(f"Train samples: {len(train_indices)}, Test/Val samples: {len(test_indices)}")
    ...
    best_test_wa = 0
    best_epoch = 0
    model_save_path = os.path.join(output_dir, f"fold_{target_fold}_best.pt")
    ...
    logging.info(f"\nFold {target_fold} Final Results (Best epoch: {best_epoch}):")
    logging.info(f"  Test WA: {final_test_wa:.2f}%")
    logging.info(f"  Test UA: {final_test_ua:.2f}%")
    logging.info(f"  Test W-F1: {final_test_wf1:.2f}%")
    
    # 结果写入单个文件
    results_file = os.path.join(output_dir, f"fold_{target_fold}_results.log")
    with open(results_file, 'w') as f:
        f.write(f"Fold {target_fold} Results\n")
        f.write("="*30 + "\n")
        f.write(f"WA: {final_test_wa:.2f}%\n")
        f.write(f"UA: {final_test_ua:.2f}%\n")
        f.write(f"W-F1: {final_test_wf1:.2f}%\n")
        f.write(f"Best Epoch: {best_epoch}\n")

import argparse # 添加在文件顶部

if __name__ == '__main__':
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="MMER Single Fold Training")
    parser.add_argument('--fold', type=int, required=True, choices=[1, 2, 3, 4, 5], 
                        help='Specify which fold to run (1-5)')
    ...
    # 建议输出目录名字里包含fold信息，防止多进程同时运行时相互冲突
    output_dir = os.path.join("outputs", current_date, f"{current_time}_fold{args.fold}")
    os.makedirs(output_dir, exist_ok=True)
    
    # 配置日志 (名字带上fold编号)
    script_name = os.path.splitext(os.path.basename(sys.argv[0]))[0]
    log_file = os.path.join(output_dir, f"{script_name}_fold{args.fold}.log")
    ...
    logging.info(f"Starting MMER training for FOLD {args.fold} with seed={args.seed}...")
    train_single_fold(target_fold=args.fold, output_dir=output_dir, seed=args.seed)

    logging.info(f"Fold {args.fold} Training completed!")
