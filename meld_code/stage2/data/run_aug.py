        ...
        if use_backup:
            ...
            if key in speaker_emotion_refs:
                ...
                if os.path.exists(current_original_wav):
                    ...
                    print(f"\n❌ 严重错误: 找不到源文件且无参考音频兜底 -> {current_original_wav}")
...
if __name__ == "__main__":
    ...
    INPUT_CSV = os.path.join(base_dir, f"data/aug_meld_processed/{split}/meld_{split}_augmented.csv")
    OUTPUT_AUDIO_DIR = os.path.join(base_dir, f"data/aug_meld_processed/{split}/wavs")
    AUDIO_ROOT_DIR = os.path.join(base_dir, f"data/meld_processed/{split}/wavs")
    
    if not os.path.exists(INPUT_CSV):
        print(f"❌ 找不到输入文件: {INPUT_CSV}，请确保已执行回译脚本。")
    else:
