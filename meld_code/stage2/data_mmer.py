...
class MELDMMERDataset(Dataset):
    ...
    def __init__(self, csv_path, audio_root_dir, roberta_path, 
        ...
        print(f"Dataset loaded from {csv_path}: {len(self.df)} samples")
    ...
    def __getitem__(self, idx):
        ...
        # 情感标签转换
        emotion_label = self.emotion_to_id.get(emotion, 0) # 默认归为 neutral
        asr_indices = self.text_to_indices(text)
